...
lwan_response(struct lwan_request *request, enum lwan_http_status status)
{
    ...
    if (UNLIKELY(request->flags & REQUEST_IS_WEBSOCKET)) {
        lwan_status_debug("WebSocket request, ignoring call");
        return;
    }
...
static void lwan_response_websocket_write_internal(struct lwan_request *request,
                                                   ...
                                                   size_t len)
{
    ...
    vec[last++] =
        (struct iovec){.iov_base = &header_byte, .iov_len = 1};

    if (len <= 125) {
        vec[last++] = (struct iovec){.iov_base = &len, .iov_len = 1};
    } else if (len <= 65535) {
...
void lwan_response_websocket_write(struct lwan_request *request)
{
    ...
    if ((request->flags & REQUEST_IS_WEBSOCKET) != REQUEST_IS_WEBSOCKET)
        return;

    lwan_response_websocket_write_internal(request, header, msg, len);
    lwan_strbuf_reset(request->response.buffer);
}
...
static void send_websocket_pong(struct lwan_request *request, size_t len)
{
    ...
    lwan_response_websocket_write_internal(request, WS_OPCODE_PONG, temp, len);
    /* FIXME: force GC here of pre-malloc gen?  Use VLA? */
...
next_frame:
    ...
    case WS_OPCODE_CLOSE:
        ...
        fin = true; /* FIN has not been received, but connection has been
                       closed. Signal to not `goto next_frame` */
        ret = false;
        break;
    case WS_OPCODE_PING:
    ...
    /* FIXME: check overflow below vvvv */
    if (UNLIKELY(!lwan_strbuf_grow_to(request->response.buffer, cur_len + len_frame))) {
        coro_yield(request->conn->coro, CONN_CORO_ABORT);
        __builtin_unreachable();
    }
    ...
    if (LIKELY(header & 0x80)) {
         ...
         * don't assume */
        struct iovec vec[] = {
            {.iov_base = masks, .iov_len = sizeof(masks)},
