...
parse_headers(struct request_parser_helper *helper, char *buffer, char *buffer_end)
{
    for (char *p = buffer; *p; buffer = ++p) {
        ...
        case MULTICHAR_CONSTANT_L('C','o','n','t'):
            ...
            break;
        CASE_HEADER(MULTICHAR_CONSTANT_L('U','p','g','r'), "Upgrade")
            helper->upgrade.value = value;
            helper->upgrade.len = length;
            break;
        case MULTICHAR_CONSTANT_L('S','e','c','-'):
            p += sizeof("Sec-WebSocket") - 1;

            STRING_SWITCH_L(p) {
            CASE_HEADER(MULTICHAR_CONSTANT_L('-','K','e','y'), "-Key")
                helper->sec_websocket_key.value = value;
                helper->sec_websocket_key.len = length;
                break;
            }
            break;
...
lookup_again:
    ...
    if (UNLIKELY(status != HTTP_OK)) {
        if (!(request->flags & REQUEST_IS_WEBSOCKET)) {
            lwan_default_response(request, status);
            goto out;
        }
    }
